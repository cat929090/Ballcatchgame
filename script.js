/* First-person Cursor Throw & Catch — Windows 98 cursor visuals (updated visuals)
   - Center cursor interior white, outline black
   - Blue background handled in CSS
   - Ball color set to bright red for visibility
*/
...
modeSelect && modeSelect.addEventListener('change', e => { mode = modeSelect.value; resetGame(); });
...
const BALL_COLOR = '#ff2b2b'; // red ball
...
function resize(){ W = canvas.width = innerWidth; H = canvas.height = innerHeight; if (players && players[0]) { players[0].x = W/2; players[0].y = H/2; } }
...
function makePlayers(){
  players[0] = {
    ...
    color:'#000000', fill:'#ffffff', hasBall: true, charging:false, chargeStart:0, recentAims: [], isHuman:true
  };
  players[1] = {
    id:1, x: W*0.8, y: H*0.5, color:'#000000', fill:'#ffffff', hasBall: false, charging:false, chargeStart:0,
...
function spawnBall(fromIdx, speed, curve){
  ...
  ball = { x: from.x, y: from.y, vx, vy, r: BALL_RADIUS, owner: fromIdx, target: 1 - fromIdx, state: 'outgoing', traveled:0, maxDistance: Math.min(1400, 200 + speed*0.6), curve: curve || 0, born: now() };
  players[fromIdx].hasBall = false; hideCatchButton();
}

// Mouse/touch aim handling
canvas.addEventListener('mousemove', e => { const rect = canvas.getBoundingClientRect(); mouseAim.x = e.clientX - rect.left; mouseAim.y = e.clientY - rect.top; const r = players[0].recentAims; r.push({x:mouseAim.x, y:mouseAim.y, t: now()}); if (r.length > 12) r.shift(); });
canvas.addEventListener('mousedown', e => { if (gameOver) return; if (!players[0].hasBall) return; players[0].charging = true; players[0].chargeStart = now(); players[0].recentAims.length = 0; });
canvas.addEventListener('mouseup', e => { if (!players[0].charging) return; players[0].charging = false; if (!players[0].hasBall) return; const dt = Math.min(MAX_CHARGE, now() - players[0].chargeStart); const t = dt / MAX_CHARGE; const speed = MIN_THROW_SPEED + t * (MAX_THROW_SPEED - MIN_THROW_SPEED); let curve = 0; const rp = players[0].recentAims; if (rp.length >= 2){ const first = rp[0], last = rp[rp.length-1]; const mvx = last.x - first.x; curve = Math.max(-1, Math.min(1, mvx / 200)) * (0.8 + t*1.2); } spawnBall(0, speed, curve); });

// Touch support
canvas.addEventListener('touchstart', e => { if (gameOver) return; e.preventDefault(); const t = e.changedTouches[0]; touchActive = true; touchId = t.identifier; const rect = canvas.getBoundingClientRect(); mouseAim.x = t.clientX - rect.left; mouseAim.y = t.clientY - rect.top; players[0].recentAims.push({x:mouseAim.x,y:mouseAim.y,t:now()}); if (players[0].recentAims.length>12) players[0].recentAims.shift(); if (!players[0].hasBall) return; players[0].charging = true; players[0].chargeStart = now(); }, {passive:false});
...
// clicking/tapping to catch for player1
canvas.addEventListener('click', e => { if (!ball) return; if (ball.state !== 'incoming') return; if (ball.target !== 0) return; const rect = canvas.getBoundingClientRect(); const d = Math.hypot(ball.x - players[0].x, ball.y - players[0].y); if (d <= ball.r + 30) catchBall(0); });
...
function drawWin98Cursor(x,y,scale, fill, stroke){
  ...
  for (let row=0; row<map.length; row++){
    for (let col=0; col<map[row].length; col++){
      if (map[row][col] === '1') ctx.fillRect(x + (col-2)*s, y + (row-8)*s, s, s);
    }
  }
  // inner fill (white)
  ctx.fillStyle = fill;
...
function draw(){ ctx.clearRect(0,0,W,H); ctx.fillStyle = 'transparent'; ctx.fillRect(0,0,W,H);
  // draw opponent larger and visible
  const p = players[1]; if (p){ drawWin98Cursor(p.x, p.y, 4, '#ffffff', '#000000'); ctx.beginPath(); ctx.strokeStyle='rgba(255,255,255,0.06)'; ctx.arc(p.x,p.y,32,0,Math.PI*2); ctx.stroke(); if (p.hasBall){ ctx.beginPath(); ctx.fillStyle=BALL_COLOR; ctx.arc(p.x+34,p.y-14,10,0,Math.PI*2); ctx.fill(); } }
  // draw ball (red)
  if (ball){ ctx.beginPath(); ctx.fillStyle = 'rgba(0,0,0,0.35)'; ctx.ellipse(ball.x+6, ball.y+8, ball.r*0.9, ball.r*0.45, 0,0,2*Math.PI); ctx.fill(); ctx.fillStyle = BALL_COLOR; ctx.beginPath(); ctx.arc(ball.x, ball.y, ball.r,0,Math.PI*2); ctx.fill(); ctx.beginPath(); ctx.fillStyle='rgba(255,255,255,0.22)'; ctx.arc(ball.x-4, ball.y-6, ball.r*0.35,0,Math.PI*2); ctx.fill(); }
  // draw center Win98 cursor (player1) bigger and white interior
  const center = players[0]; drawWin98Cursor(center.x, center.y, 6, '#ffffff', '#000000');
  // draw charge arc
  if (center.charging && center.hasBall){ const dt = Math.min(MAX_CHARGE, now() - center.chargeStart); const t = dt / MAX_CHARGE; const ang = Math.PI*2 * t; ctx.beginPath(); ctx.strokeStyle = 'rgba(255,255,255,0.9)'; ctx.lineWidth = 6; ctx.arc(center.x, center.y, 54, -Math.PI/2, -Math.PI/2 + ang); ctx.stroke(); }
  // HUD subtle
  ctx.fillStyle = 'rgba(0,0,0,0)'; ctx.fillRect(8, H-60, 260, 48);
}
